...
module SeIntroGeom.Functions 
open System

type private Rect = int * int * int * int
...
module Res = 
    let isRectIntersect (x1, y1, x2 y2) (x1', y1', x2', y2') = Option.isSome (segInt1d x1 x2 x1' x2') && Option.isSome (segInt1d y1 y2 y1' y2')
    let isCircleIntersect (x1, y1, r1) (x2, y2, r2) = 
        let dx, dy = x1 - x2, y1 - y2
        let d = dx * dx + dy * dy
        d <= r1 + r2 && r1 <= d + r2 && r2 <= r1 + d
    let lineContainsPoint = 
        //match resolveIntersections (Intersect) with
       // |
    let isSegsIntersect = mathc Intersect (LineSegment (), LineSegment ())
    let isPointInTriangle = 
    let isPointInEllipse = 
    let isTriangleSidesEqual = 
    let isTriangleRight = 
    let isParallelogram = 
    let isConvex = 
    let isSegCircleIntersect = 
    let resolveIntersections g = SeIntroGeom.Functions 
